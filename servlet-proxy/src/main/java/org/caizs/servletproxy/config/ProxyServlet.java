...
public class ProxyServlet extends HttpServlet {
  ...
  /* MISC */
    ...
    protected List<String> exludeUrls = new ArrayList<>();
    protected final PatternMatcher pathMatcher = new ServletPathMatcher();
    ...
    public void init() throws ServletException {
        ...
        String excludes = getConfigParam(P_EXCLUDE);
        if (excludes != null) {
            this.exludeUrls = getExludeUrls(excludes);
        }
    ...
    private List<String> getExludeUrls(String excludes) {
    ...
    protected RequestConfig buildRequestConfig() {
        RequestConfig.Builder builder = RequestConfig.custom()
                                                     ...
                                                     .setCookieSpec(
                                                             CookieSpecs.IGNORE_COOKIES) // we handle them in the servlet instead
    ...
    protected void initTarget() throws ServletException {
        ...
        //test it's valid
        try {
            targetUriObj = new URI(Optional.ofNullable(getConfigParam(P_HOST)).orElse(targetUri));//覆盖host
        } catch (Exception e) {
    ...
    private boolean isExcludeURL(String currentURL) {
        for (String url : this.exludeUrls) {
            if (pathMatcher.matches(url, currentURL)) {
                return true;
            }
        }
        return false;
    ...
    protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
        ...
        String currUrl = getUrl(servletRequest);
        if (isExcludeURL(currUrl)) {
            super.service(servletRequest, servletResponse);//TODO 调整controller
        }
    ...
    protected void copyProxyCookie(HttpServletRequest servletRequest,
        ...
        for (HttpCookie cookie : cookies) {
            ...
            String proxyCookieName = doPreserveCookies ?
                    cookie.getName() :
